...
require_once("ornek.php");
    ...
    $sonuc = mysqli_query($baglanti, " SELECT  AVG(orderdetails.quantityOrdered) AS avg
FROM `products` INNER JOIN `productlines` INNER JOIN `customers` INNER JOIN `orders` INNER JOIN `orderdetails` 
ON products.productLine = productlines.productLine AND customers.customerNumber = orders.customerNumber AND orders.orderNumber = orderdetails.orderNumber AND products.productCode = orderdetails.productCode AND products.buyPrice
  ...
  echo"<p class='price'>";
      ...
      printf ("Müşteri İsmi:". " " .$satir["contactFirstName"]);  
        echo"</p>";
           ...
           echo"<p>";
      printf ("Sipariş Miktarı ortalaması". " " .$satir["avg"]);  
        echo"</p>";
